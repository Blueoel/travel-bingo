...
사용자는 지역을 검색하거나 추천받아 역사·공원·카페·자연·야경·맛집·축제 등 여러 개의 5×5 테마 빙고를 플레이한다. 현장에서 GPS·QR·퀴즈·사진으로 미션을 인증하고, 3개 빙고 라인을 완성하면 해당 판을 클리어한다. 지역 빙고를 3 Bingo로 클리어하면 탐험 탭의 해당 행정구역이 해금되고, 사용자가 고른 인증 사진으로 지역 모양을 채울 수 있다. 여행과 재방문이 쌓일수록 대한민국 지도가 개인 사진과 동·은·금 테두리로 완성된다. 지역 운영자는 한국관광공사 관광데이터를 바탕으로 상시 테마 빙고와 기간 한정 이벤트 빙고를 제작할 수 있다.
...
탐험 탭은 지역 검색을 위한 일반 지도 화면이 아니라, 지역 빙고를 완수하며 사용자의 사진으로 채워가는 **개인 대한민국 여행 지도**다. 가족·친구·혼자 다녀온 여행의 인증 사진을 행정구역 모양 안에 남겨, 시간이 흐를수록 `내가 다녀온 대한민국 앨범`이 완성되게 한다. 지역 검색과 한국관광공사 관광지 추천은 홈 또는 별도의 검색 기능에서 제공하고, 탐험 탭은 성취·수집·추억 회상에 집중한다.

#### 지역 해금 규칙

1. 사용자가 지역 빙고판에서 서로 다른 3개 라인을 완성해 `CLEAR(3 Bingo)`를 달성한다.
2. 서버가 해당 빙고판의 지역과 클리어 상태를 확정하고 지역 탐험 보상을 지급한다.
3. 탐험 탭에서 해당 행정구역이 사진을 넣을 수 있는 상태로 해금된다.
4. 사용자는 그 지역 빙고에서 제출한 사진 중 한 장을 **대표 추억 사진(My Moment)**으로 선택한다.
5. 선택한 사진을 행정구역 경계로 마스킹해 지도 안에 표시한다.
6. 대표 사진은 언제든 교체할 수 있고, 이전 사진은 지역 여행 기록에 보존한다.

사진 미션이 없는 빙고판을 클리어했거나 적절한 인증 사진이 없을 때는 해금 후 별도의 대표 여행 사진 업로드를 허용한다. 이때도 해당 지역 빙고의 3 Bingo 클리어 기록이 있어야 지도에 사진을 넣을 수 있다.

#### 탐험 단계와 지역 테두리

지역 테두리는 단순 누적 미션 수가 아니라, **서로 다른 지역 테마 빙고판을 3 Bingo로 클리어한 횟수**를 기준으로 성장한다.

| 지역 내 클리어한 테마 빙고판 | 지도 상태 | 표현 | 의미 |
|---:|---|---|---|
| 0개 | 미해금 | 흰색 또는 종이 질감 | 아직 지도 앨범에 기록되지 않은 지역 |
| 1개 | 브론즈 | 대표 사진 + 동색 테두리 | 첫 지역 빙고 여행 완료 |
| 2개 | 실버 | 대표 사진 + 은색 테두리 | 서로 다른 두 테마로 지역 재탐험 |
| 3개 이상 | 골드 | 대표 사진 + 금색 테두리 | 세 가지 이상의 테마 빙고를 완수한 지역 마스터 |

초기 MVP에서는 안성 지역의 빙고판 1개를 3 Bingo로 클리어하면 사진과 동색 테두리를 해금하는 흐름까지 구현한다. 은색은 서로 다른 안성 테마 빙고판 2개, 금색은 서로 다른 테마 빙고판 3개를 각각 3 Bingo로 클리어했을 때 지급한다. 동일 빙고판 반복 클리어는 테두리 승급 횟수에 중복 집계하지 않는다.

테두리는 사진을 대체하지 않는 별도의 장식 레이어이며, 색상만으로 단계를 구분하지 않도록 `브론즈·실버·골드` 텍스트, 배지 또는 아이콘을 함께 표시한다.

#### 탐험 탭 사용자 경험

- 전국 시·군·구 단위 여행 지도와 해금 현황
- 사진으로 채운 지역 수와 브론즈·실버·골드 지역 수
- 해금됐지만 대표 사진을 아직 고르지 않은 지역 안내
- 지역 선택 시 대표 사진, 여행 날짜, 한 줄 기록 표시
- 해당 지역에서 완료·진행 중인 테마 빙고 목록
- 다음 테두리 단계까지 필요한 서로 다른 테마 빙고 수
- 지역별 인증 사진 모아보기와 대표 사진 변경
- 개인 여행 지도를 한 장의 이미지로 저장·공유

예를 들어 안성의 첫 테마 빙고를 3 Bingo로 클리어하면 안성 영역이 해금된다. 사용자가 `안성맞춤랜드` 인증 사진을 대표 사진으로 고르면 안성 행정구역 내부에 사진이 표시되고 동색 테두리가 생긴다. 이후 안성의 다른 테마 빙고를 클리어하면 은색, 세 번째 서로 다른 테마 빙고를 클리어하면 금색 테두리로 승급한다.

#### 지도 구현 방식

지도는 라이선스가 확인된 SVG 또는 GeoJSON 행정구역 경계를 `Region.administrativeCode`와 연결한다. 사진은 SVG `clipPath`·`pattern`, Canvas 마스킹 또는 동등한 방식으로 지역 경계 안에 `cover` 형태로 배치한다. 얼굴처럼 중요한 피사체가 잘리지 않도록 사용자가 사진의 확대 비율과 초점을 조정한 뒤 저장할 수 있게 한다.

원본 사진을 전국 지도 화면에 직접 내려보내지 않고 작은 WebP 썸네일과 서명 URL을 사용한다. 지도에는 정밀 GPS 좌표와 원본 EXIF를 포함하지 않는다. 데스크톱에서는 지도와 선택 지역 상세 패널을 함께 표시하고, 모바일에서는 전체 지도에서 지역을 누르면 하단 시트 또는 지역 추억 상세 화면을 연다.

시·군·구 경계 데이터는 출처, 사용 허가, 기준연도와 행정구역 개편 이력을 관리한다. 경계가 변경되더라도 기존 여행 기록은 당시 지역 코드와 현행 지역 코드의 매핑을 통해 보존한다.

### 4.8 여행 기록과 공유 커뮤니티

3 Bingo로 해금한 지역마다 사용자가 가장 마음에 들었던 사진 한 장을 **나의 순간(My Moment)**으로 지정하고 짧은 기록을 남긴다. 이는 지역을 객관적으로 대표하는 사진이 아니라 그 지역에서 사용자가 가장 간직하고 싶은 개인적인 순간이며, 해당 지역 여행 일기의 표지이자 탐험 지도 내부를 채우는 사진이다.
...
나의 순간은 해당 지역의 인증 사진 또는 해금 후 사용자가 추가한 여행 사진 중에서 직접 선택한다. 안성에서 50장의 사진을 남겼다면 그중 한 장과 한 줄 기록을 사용자가 고른다. 언제든 나의 순간을 바꿀 수 있으며 변경 전 사진과 과거 여행 기록은 앨범에 유지한다. 최근 사진·즐겨찾기 사진을 후보로 추천할 수 있지만 최종 선택은 반드시 사용자에게 맡긴다.
...
- 지역 테마 빙고판 1개·2개·3개를 각각 3 Bingo로 클리어하면 해당 지역 지도 테두리를 동색·은색·금색으로 단계 승급한다.
- 시·도 또는 전국 도감 마스터 보상은 지역 테두리와 혼동되지 않는 별도의 외곽 장식·칭호·배지로 표시한다.
...
동·은·금 테두리는 대표 사진을 대체하지 않는 별도 장식 레이어로 렌더링한다. 사용자가 접근성 설정에서 애니메이션 감소를 선택하면 정적인 완료 카드로 대체한다.
...
| 대한민국 탐험 지도 | 라이선스가 확인된 행정구역 SVG/GeoJSON + SVG 마스킹 | 3 Bingo 지역 해금, 대표 사진 채우기, 동·은·금 테두리 표현 |
...
| RegionExplorationReward | userId, regionId, clearedTemplateCount, borderTier(NONE/BRONZE/SILVER/GOLD), unlockedAt, tierUpdatedAt |
| RegionTemplateClear | userId, regionId, templateId, sessionId, clearType(THREE_BINGO/PERFECT), clearedAt |
...
| MyMomentCrop | myMomentId, scale, focusX, focusY, boundaryVersion, renderedThumbnailKey |
...
- 지역 탐험 보상: `LOCKED → BRONZE → SILVER → GOLD`
...
| GET | `/api/v1/map/progress` | 지역별 3 Bingo 클리어 수와 LOCKED/BRONZE/SILVER/GOLD 상태 조회 |
| GET | `/api/v1/map/album` | 해금 지역의 나의 순간 썸네일·사진 초점·테두리 단계 조회 |
...
| GET | `/api/v1/users/me/regions/{regionId}/moment-candidates` | 해당 지역 인증 사진과 업로드 사진 중 대표 사진 후보 조회 |
| PUT | `/api/v1/users/me/regions/{regionId}/my-moment` | 해금 지역의 나의 순간 사진·장소·날짜·문장·크롭 초점 선택 |
...
- 단위 테스트: 거리 계산, 12개 라인 및 3빙고 클리어 판정, 서로 다른 테마 클리어 중복 방지, 브론즈·실버·골드 승급, 점수, 업적 조건, 이벤트 순위 동률 처리
...
- 탐험 지도 E2E 테스트: 지역 3 Bingo 클리어 → 지역 해금 → 인증 사진 선택 → 초점 조정 → 지도 사진 마스킹 → 동색 테두리 → 비공개 정보 제외 공유
- 도감 테스트: 서로 다른 테마 빙고 1·2·3개 클리어 집계, 동·은·금 승급, 다음 목표 추천, 프로필 전시
...
- 지역별 탐험 지도 전환율(미해금→브론즈→실버→골드)
- 테마 시작률, 3빙고 클리어율, 퍼펙트 클리어율
- 여행 기록 작성률, 해금 후 나의 순간 선택률, 사진 지도 재열람률, 공개·공유 전환율과 신고율
