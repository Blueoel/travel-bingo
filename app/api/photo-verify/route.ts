  ...
  try {
    ...
    if (!match?.[1] || !match[2]) {
      return NextResponse.json(
        { code: "INVALID_IMAGE", message: "JPEG, PNG, WebP 사진만 사용할 수 있어요." },
        { status: 400 },
      );
    }
    ...
    if (!apiKey) {
      return NextResponse.json(
        { code: "AI_NOT_CONFIGURED", message: "사진 AI 인증이 아직 설정되지 않았어요." },
        { status: 503 },
      );
    }
            ...
            parts: [
              {
                text: [
                  ...
                  "분명히 충족하면 APPROVED, 분명히 불충족하면 REJECTED,",
    ...
    if (!response.ok) {
      return NextResponse.json(
        { code: "AI_PROVIDER_ERROR", message: "AI 사진 판정을 완료하지 못했어요." },
        { status: 502 },
      );
    }
    ...
    const verdict = parseVerdict(part?.text);
    return NextResponse.json({ ...verdict, model });
  } catch {
    return NextResponse.json(
      { code: "AI_VERIFICATION_FAILED", message: "사진 인증 중 오류가 발생했어요." },
      { status: 500 },
    );
  }
}
...
function parseVerdict(value: unknown): GeminiVerdict {
  ...
  if (
    !["APPROVED", "REJECTED", "NEEDS_REVIEW"].includes(String(parsed.decision)) ||
