...
export default function Home() {
  ...
  const saveNickname = async () => {
    setSettingsSaving(true); setSettingsStatus(null);
    try {
      ...
      setAccount(payload.user); setNickname(payload.user.nickname); setProfileNickname(payload.user.nickname); setSettingsStatus("닉네임을 변경했어요.");
    } catch (error) { setSettingsStatus(error instanceof Error ? error.message : "닉네임을 변경하지 못했어요."); }
  ...
  const savePassword = async () => {
    ...
    try {
      ...
      window.alert("비밀번호를 변경했어요. 새 비밀번호로 다시 로그인해주세요."); logout();
    } catch (error) { setSettingsStatus(error instanceof Error ? error.message : "비밀번호를 변경하지 못했어요."); }
    finally { setSettingsSaving(false); }
  };
  const withdrawAccount = async () => {
    ...
    } catch (error) { setSettingsStatus(error instanceof Error ? error.message : "회원 탈퇴를 처리하지 못했어요."); }
    finally { setSettingsSaving(false); }
  };
  useEffect(() => { if (activeTab === "my" && authStatus === "authenticated") void loadAccountSummary(); }, [activeTab, authStatus]);
          ...
          {myView === "travel-note" ? (
            ...
            <div className="settings-view">
              <section className="account-settings-card">
                ...
                {account?.email && <><label>현재 비밀번호<input type="password" value={currentPassword} autoComplete="current-password" onChange={(event) => setCurrentPassword(event.target.value)} /></label><label>새 비밀번호<input type="password" value={newPassword} minLength={8} autoComplete="new-password" placeholder="8자 이상" onChange={(event) => setNewPassword(event.target.value)} /></label><button type="button" disabled={settingsSaving || !currentPassword || newPassword.length < 8} onClick={() => void savePassword()}>비밀번호 변경</button></>}
